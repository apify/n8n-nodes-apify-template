...
```bash
mkdir -p ~/.n8n/custom
ln -s /full/path/to/n8n-nodes-apify-rag-web-browser ~/.n8n/custom/n8n-nodes-apify-rag-web-browser # replace full/path/to with the path to your n8n-nodes-apify-rag-web-browser directory
```
...
1.  **Automated Workflow Execution:**
    ...
    * It will perform:
        ...
        6.  Publish the package `@apify/n8n-nodes-apify-rag-web-browser@X.Y.Z` to npm.
...
2.  **Verify the Package on npm:**
    ...
    * Verify the new version on npm:
        ```bash
        pnpm view @apify/n8n-nodes-apify-rag-web-browser version
        ```
...
3. Open an issue in the [GitHub repository](https://github.com/apify/n8n-nodes-apify-rag-web-browser)
