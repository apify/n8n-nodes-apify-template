...
### 2. Check out the SNIPPET list of important code

We highly recommend checking out outlined parts of code you might want to edit. You can find the locations of code snippets 
by performing text search (Example: `SNIPPET 1`) in your IDE. 

1. **Actor Schema Constants:** 
Location: Search for `SNIPPET 1` in your IDE or go to /nodes/<YourActorName>/<YourActorName>.node.ts
Check out the constants generated by the Actor Schema.
...
2. **Change Icon**  
Location: Search for `SNIPPET 2` in your IDE or go to `/nodes/{YourActorName}/{YourActorName}.node.ts`. 
Adjust the logo of your node, <___> logo is the default value.    
You can also use 2 different logos for `dark` and `light` mode.

3. **Edit Subtitle Text**  
Location: Search for `SNIPPET 3` in your IDE or go to `/nodes/<YourActorName>/<YourActorName>.node.ts`. 
Adjust the subtitle of your node. It will show up under the node on the workflow screen.
![Actor Subtitle](./docs/actor-subtitle.png)

4. **Edit Description**  
Location: Search for `SNIPPET 4` in your IDE or go to `/nodes/<YourActorName>/<YourActorName>.node.ts`. 
Adjust the description of your node, Actor description is the default value.
Ideal length of the description is **1 sentence** long. It will appear on the n8n node search.
![Apify Node Description](./docs/node-description.png)

5. **AI Tool Optimizations**  
Location: Search for `SNIPPET 5` in your IDE or go to `/nodes/<YourActorName>/resources/genericFunctions.ts`. 
When an Apify Actor is called from an AI Agent on n8n it is good practice to clean the results to reduce context.
